...
public class Exam1 {
	public static void main(String[] args) {
		...
		names.stream().filter(name -> name.startsWith("kim"))
		...
		List<Integer> even_num = num.stream().map(Integer::parseInt)  
	      			.filter(n -> n % 2 == 0) 
	                .map(n -> n * n)
		...
		System.out.println("------------------");
	      ...
	      //Optionald안에 값이 있으먄 Consumer<T>를 실행하고
	      //값이 없으면 아무 일도 하지 않는다
	      Stream<Student> stream = students.stream();
	      stream.max(Comparator.comparingInt(Student::getScore))
